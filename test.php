<form action = "test.php" method = "POST" style = "display:none;" name = "test">
...
function  pop_up() {
	if(!flag) {
	    ...
	    if ((pass != null) && (pass == "asdasd")) {
	    	console.log(document.getElementById("none").click());
	        flag = true;
	    } else  {
	    	alert("Грешна парола");
	    }
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		{
				if(isset($_POST[$row[0]]) && (isset($_POST["none"])))
				{
					$name = "s" . $row[0];
	...
	while($row = mysql_fetch_row($result)) {
	   ...
	   echo"</select></form></td>";
	   echo "<td><input  class = 'button' type = 'submit' form = '$row[0]' name = '$row[0]' onclick = 'pop_up()' value = 'Обнови'> </td>";
	   echo "</tr>";
	   $count++;
	}	
?>
